...
These tests validate that the classify function correctly categorizes
Jeopardy questions into numbers, non-English content, and unusual proper nouns.
"""
...
TEST_CASES = [
    ...
    (
        ["The temperature was 98.6 degrees", "Price dropped to $19.99 today", "Meeting at 3:30 PM on January 1st", "Born in 1985 and graduated in 2007"],
        [0, 1, 2, 3], [2], []  # "PM" detected as non-English
    ),
    
    # Foreign language content
    (
        ["Say hello in Spanish: hola", "French word for cat: chat", "German greeting: guten tag"],
        [], [0, 2], []  # "chat" not detected as foreign
    ),
    
    # Mixed foreign languages
    (
        ["Say konnichiwa in Japanese culture", "Italian ciao means hello or goodbye", "Russian dasvidaniya means farewell"],
        [], [0, 2], [2]  # "dasvidaniya" also detected as unusual proper noun
    ),
    
    # Made-up fictional names
    (
        ["The wizard Zorkblatt cast spells", "Princess Xyrellia ruled the kingdom", "Hero Blorzaniq saved the day"],
        [], [0, 1, 2], [0, 1, 2]
    ),
    
    # Scientific terms
    (
        ["DNA contains adenine and thymine bases", "Algorithm uses machine learning techniques", "Photosynthesis occurs in chloroplasts"],
        [], [], []
    ),
    ...
    (
        ["In 1969 astronauts said bonjour to aliens", "The wizard Krythaxios cast 7 magic spells", "Scientist discovered 42 new especies today"],
        [0, 1, 2], [0, 1, 2], [1]
    ),
...
@pytest.mark.parametrize("questions,expected_numbers,expected_non_english,expected_unusual_proper_nouns", TEST_CASES)
def test_classify_comprehensive(questions, expected_numbers, expected_non_english, expected_unusual_proper_nouns):
